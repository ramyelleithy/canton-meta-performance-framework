...
### Top recurring destination (added 2026-08-02)

**Website only — Confirmed by structural evidence, not by a direct `destination_type` field pull (that field is Unknown — see below).** Re-verified directly against the account on 2026-08-02, prompted by a Page-level "Browser add-ons → WhatsApp" setting surfacing in Canton's Facebook Page settings (WhatsApp number +20 1515688732 connected to Page `1781350502173727`). Three independent, structural pieces of evidence — pulled fresh from the account, not from the earlier Historical Pattern Discovery pass — indicate the historical winning campaigns drove to the website, not to WhatsApp:

1. **Optimization goal `OFFSITE_CONVERSIONS` is Confirmed 100% (5/5) across the top-5 ad sets** (see "Top recurring optimization" above). This optimization goal binds delivery to a Meta Pixel Purchase event fired on `canton-eg.com`; a WhatsApp-destined ad set cannot fire that event, so this goal is structurally incompatible with a WhatsApp-only or WhatsApp-first funnel.
2. **Attribution `1d_view_7d_click_1d_ev` is Confirmed 100% (5/5)** — this window measures click/view-to-website-purchase, again requiring a website destination to be meaningful.
3. **Ad copy on the two documented Benetton winning creatives** (`docs/winners.md` Winner 2, ad set `120241892188310716`, creatives `1266949824918428` and `1467282721565667`, both re-pulled 2026-08-02) explicitly reads: *"للطلب والتفاصيل اضغطي على الرابط أسفل الصور"* ("to order, click the link below the pictures") — link/website language, not "message us on WhatsApp."

**Caveat — genuinely Unknown, not assumed:** `call_to_action_type` and `link_url` were re-pulled directly from both winning creatives above via `ads_get_creatives` on 2026-08-02 and came back empty for both — consistent with the existing "Top recurring CTA" finding (Unknown, 0/5 determinable). Separately, `url_tags` (the field that would carry UTM parameters at ad/adset level) and `destination_type` were checked against the tool's own field catalog via `ads_get_field_context` on 2026-08-02 and returned as `unknown_fields` — **these fields are not exposed by the connected Marketing API tool at all, at any level.** This is a stronger form of "Unknown" than an empty value: it means UTM parameters and destination_type can never be directly confirmed or denied through this toolset, not just that no value happened to be set on these particular creatives. The Marketing API cannot directly confirm the CTA button type, destination URL, or UTM structure on these STATUS/Page-Post creatives. **No evidence of any kind — direct or structural — was found that any top-5 winning ad set or ad used WhatsApp as a destination.** The Page-level WhatsApp button setting shown in the 2026-08-02 screenshot is a Page-wide "Browser add-on" (organic Page CTA), not an ad-level destination_type, and there is no evidence it was active during, or attributable to, any of the winning campaigns' results.

**Decision for future campaign builds:** until a controlled experiment says otherwise, new campaigns should set destination = Website only, matching the structural pattern of every documented winner. Do not add a WhatsApp/Messenger destination or CTA to a Purchase-objective campaign without a new logged experiment, per Evidence Rules Rule 7 (every framework modification requires Historical Evidence, a Controlled Experiment, or a Verified Winner).
